...
# WALを使えないRenderの永続ディスクでは、複数スレッド（gthread＋通知＋バックアップ）が
# 同時に書き込むと SQLite が 'database is locked' を返すことがある（busy_timeoutでも
# write同士の取り合いで即エラーになる場合がある）。そこで、execute/commit が locked で
# 失敗したら短い間隔で数秒リトライする。相手の書き込みは一瞬で終わるので、まず成功する。
# ロックを跨いで保持しない方式なので、デッドロックの危険が無い（保持型ロックは×）。
def _retry_on_locked(fn, *a, **k):
    delay = 0.04
    for i in range(60):  # 合計でおおむね最大10秒強まで粘る
        try:
            return fn(*a, **k)
        except sqlite3.OperationalError as e:
            if "locked" in str(e).lower() and i < 59:
                time.sleep(delay)
                delay = min(delay * 1.3, 0.4)
                continue
            raise


class _RetryConn:
    """sqlite3.Connection の薄いラッパー。execute/commit 等が 'database is locked' で
    失敗したら自動リトライする。読み取りも書き込みも同様に粘るので、並行アクセスでの
    瞬間的なロック衝突を吸収できる。"""
    ...
    def execute(self, *a, **k):
        return _retry_on_locked(self._c.execute, *a, **k)

    def executemany(self, *a, **k):
        return _retry_on_locked(self._c.executemany, *a, **k)

    def executescript(self, *a, **k):
        return _retry_on_locked(self._c.executescript, *a, **k)

    def commit(self):
        return _retry_on_locked(self._c.commit)
...
def _connect():
    """SQLite接続を統一設定で開く（書き込み直列化ラッパー付き）。
    ・timeout=15 / busy_timeout=15000：瞬間のロック待ちを吸収（どのFSでも安全）。
    ・_SerializedConn：書き込みをプロセス内で直列化し database is locked を防ぐ。
